...
수업 사이트 **Comfy Classroom** (`175.117.33.139:6003`) 의 로그인 화면에서 뽑아낸 디자인 언어를 따랐다.

| 토큰 | 값 |
| --- | --- |
| 배경 / 글자 | `#f6f8fc` / `#17213c` |
| 네이비 패널 | `#182d69` |
| 프라이머리 | `#254bdb` (hover `#18349f`) |
| 모서리 | `14px` |
| 서체 | DM Sans + Noto Sans KR |

왼쪽 네이비 패널 + 오른쪽 흰 작업 영역의 2단 구성, 밑줄 탭, `01`/`02`/`03` 단계 표기도 같은 방식이다.
